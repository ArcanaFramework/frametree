...
explicitly use the ``frametree.core.mark.column`` function to include some basic
...
or ``frametree menu`` CLI command. The row row_frequency of the column (e.g. per-session,
...
provided to the :func:`frametree.core.mark.analysis` class decorator.
...
Values for ``salience`` must be drawn from the :class:`frametree.core.enum.ColumnSalience` enum:
...
saliences are drawn from :class:`frametree.core.enum.ParameterSalience` enum.
...
derive data columns. The :func:`frametree.core.mark.pipeline`
...
.. code-block:: python
    ...
    from frametree.core import mark
    from frametree.core.tasks.misc import ExtractFromJson
    from frametree.core.data.salience import ColumnSalience as ds
...
.. code-block:: python
    ...
    import numpy
    import pydra.mark
    from frametree.weather.data import Weather  # See example in Data spaces section
...
.. code-block:: console
    ...
    $ frametree derive output '/data/my-dataset' connectivity_matrix_plot \
      --save '~/Documents/papers/my-connectivity-paper/' \
      --option figsize 10,10

The ``frametree.core.mark.output`` decorator is used to specify an output method
